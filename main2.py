...
# Interfaz "Developer/Engineer Mode": Muestra el proceso interno y los logs
html_content = """
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        /* Estilos para el JSON/Markdown devuelto */
...
<body class="min-h-screen">
    ...
    <nav class="border-b border-zinc-800 bg-zinc-950 px-6 py-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <div class="flex items-center gap-4 text-xs font-mono text-zinc-400">
            ...
            <span class="bg-zinc-800 px-2 py-1 rounded">v1.0 (Proof of Concept)</span>
        </div>
    </nav>
    ...
    <div class="max-w-7xl mx-auto py-8 px-4 grid lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-lg">
                ...
                <form id="uploadForm" class="space-y-4">
                    <div id="dropZone" class="border-2 border-dashed border-zinc-700 bg-zinc-950/50 p-6 rounded-lg text-center hover:border-blue-500/50 hover:bg-blue-900/10 transition-all cursor-pointer group">
                        ...
                        <p id="dropText" class="text-xs text-zinc-400 font-medium">Arrastra un documento no estructurado aquí</p>
            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-lg flex flex-col h-[400px]">
                ...
                <div class="bg-zinc-950 border border-zinc-800 rounded-lg p-4 flex-grow overflow-y-auto font-mono text-[11px] text-emerald-400/80 leading-relaxed">
                    <span class="text-purple-400">SYSTEM</span>: Eres un motor de extracción de datos especializado en perfiles IT.<br><br>
                    <span class="text-blue-400">USER</span>: Recibirás el texto de un PDF no estructurado. Tu tarea es extraer la siguiente información y formatearla estrictamente como un reporte estructurado:<br><br>
                    1. <b>Core_Stack</b>: Array de tecnologías principales.<br>
                    2. <b>Cloud_Infrastructure</b>: Proveedores y herramientas cloud.<br>
                    3. <b>0_to_1_Experience</b>: Evidencia de construcción desde cero.<br>
                    4. <b>Seniority_Signals</b>: Arquitectura, liderazgo, o toma de decisiones complejas.<br><br>
                    <span class="text-zinc-500">/* Este prompt asegura que el LLM no divague y entregue datos listos para ser consumidos por un backend. */</span>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-xl shadow-lg overflow-hidden flex flex-col h-[calc(100vh-140px)]">
                ...
                <div class="flex border-b border-zinc-800 bg-zinc-950/50 px-4">
                    <button id="tabOutputBtn" class="px-4 py-3 text-sm font-medium text-blue-400 border-b-2 border-blue-500">Structured Output</button>
                    <button id="tabLogsBtn" class="px-4 py-3 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2">
                ...
                <div class="p-6 flex-grow overflow-y-auto relative bg-[#0c0c0e]">
                    ...
                    <div id="welcomeState" class="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
                        ...
                        <p class="text-zinc-500 text-sm max-w-md">
                            Sube un PDF. El backend (FastAPI) usará PyMuPDF para vectorizar el texto y luego orquestará una llamada a Gemini 2.5 Flash para estructurar los datos.
                        </p>
                    </div>

                    <!-- Consola de Logs (simulada durante la carga) -->
                    <div id="logConsole" class="hidden absolute inset-0 bg-[#0c0c0e] p-6 font-mono text-[12px] text-zinc-400 overflow-y-auto space-y-2">
                        ...
                        <div class="flex items-center gap-2 text-blue-400 mt-2 animate-pulse">
                    ...
                    <div id="resultContainer" class="hidden">
                        ...
                        <!-- Aquí va el resultado de la IA parseado -->
    ...
    <script>
        ...
        tabOutputBtn.onclick = () => {
            if (!welcomeState.classList.contains('hidden') && logContent.innerHTML === '') return;
            tabOutputBtn.className = "px-4 py-3 text-sm font-medium text-blue-400 border-b-2 border-blue-500";
            tabLogsBtn.className = "px-4 py-3 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2";
        ...
        tabLogsBtn.onclick = () => {
            if (!welcomeState.classList.contains('hidden') && logContent.innerHTML === '') return;
            tabLogsBtn.className = "px-4 py-3 text-sm font-medium text-blue-400 border-b-2 border-blue-500";
            tabOutputBtn.className = "px-4 py-3 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2";
        ...
        form.onsubmit = async (e) => {
            ...
            // Activar visualmente la pestaña de logs al iniciar
            tabLogsBtn.className = "px-4 py-3 text-sm font-medium text-blue-400 border-b-2 border-blue-500";
            tabOutputBtn.className = "px-4 py-3 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2";

            // Simular logs en tiempo real para impresionar
            addLog("Iniciando POST /analyze/...", "text-blue-400");
            setTimeout(() => addLog("Recibiendo buffer de archivo PDF (" + fileInput.files[0].name + ")"), 400);
            setTimeout(() => addLog("Ejecutando PyMuPDF (fitz) para extracción de texto plano..."), 900);
            setTimeout(() => addLog("Limpiando saltos de línea y formateando contexto..."), 1300);
            setTimeout(() => addLog("Conectando con Google GenAI SDK (gemini-2.5-flash)...", "text-emerald-400"), 1800);
            ...
            try {
                ...
                if (data.error) {
                    ...
                    addLog("Respuesta del LLM recibida correctamente.", "text-green-400");
                    
                    // Mostrar resultados después de los logs
                    setTimeout(() => {
                        logConsole.classList.add('hidden');
                        document.getElementById('logCounter').innerText = "4";
                        
                        // Activar visualmente la pestaña de output al terminar
                        tabOutputBtn.className = "px-4 py-3 text-sm font-medium text-blue-400 border-b-2 border-blue-500";
                        tabLogsBtn.className = "px-4 py-3 text-sm font-medium text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2";
                ...
                addLog("Error crítico de red o servidor.", "text-red-500");
            } finally {
                btn.disabled = false;
...
async def analyze_document(file: UploadFile = File(...)):
    try:
        ...
        # 2. Prompt enfocado en EXTRACCIÓN DE DATOS, no en juicios de valor.
        prompt = f"""
        Eres un motor de extracción de datos para sistemas de Inteligencia de Recursos Humanos.
        Tu trabajo es analizar el texto no estructurado de un CV y devolver los datos perfectamente organizados.
        NO evalúes al candidato. Solo extrae y categoriza la información.
        ...
        (Extrae lenguajes, frameworks y bases de datos. Sé conciso).

        ### ☁️ Cloud & Infrastructure
        (Extrae herramientas de nube, despliegue, DevOps o IA que mencione).

        ### 🚀 "0 to 1" / Build Experience
        (Resume en 2 bullet points las evidencias concretas de que el candidato ha construido proyectos, features o integraciones desde cero).

        ### 🧠 Seniority Signals
        (Identifica responsabilidades avanzadas: liderazgo técnico, arquitectura de soluciones, integraciones complejas (ej. LTI/SSO), o manejo estratégico de clientes).
        """
